...
const WHY_CHOOSE_US = [
  ...
  { icon: Award, title: "High Quality Work", desc: "Workmanship guaranteed end-to-end." },
  { icon: HeartHandshake, title: "Trusted by Homeowners", desc: "1,000+ happy Singapore homes served." },
  { icon: TrendingUp, title: "End-to-End Solutions", desc: "One partner for every home project." },
];
      ...
      <div className="mx-auto max-w-5xl px-4 lg:px-6">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <div className="flex items-center justify-center gap-2 mt-4">
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="h-5 w-5 fill-[#35D6F7] text-[#35D6F7]" />
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-16">
          {WHY_CHOOSE_US.map((item) => {
            ...
            return (
              <div
                key={item.title}
                className="flex items-center gap-4 rounded-xl bg-gray-50 border border-gray-200 p-5 hover:shadow-md hover:border-[#35D6F7] transition-all"
              >
                <div className="h-12 w-12 rounded-lg bg-[#1E73D8]/10 grid place-items-center shrink-0">
                  <Icon className="h-6 w-6 text-[#1E73D8]" />
                </div>
                <div>
        ...
        <div className="grid md:grid-cols-2 gap-2">
          {REVIEWS_LIST.map((review) => (
            <article
              key={review.name}
              className="relative rounded-xl bg-white border border-gray-200 p-6 hover:shadow-lg hover:border-[#35D6F7] transition-all"
            >
              <Quote className="absolute top-4 right-4 h-8 w-8 text-[#1E73D8]/10" />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-gradient-primary text-white grid place-items-center font-semibold text-sm">
        ...
        <div className="text-center mt-12">
          ...
          >
            <Star className="h-4 w-4 fill-[#35D6F7] text-[#35D6F7]" />
